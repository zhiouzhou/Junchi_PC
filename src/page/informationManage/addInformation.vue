  ...
  <div id="addInformation">
    ...
    >
      <el-form-item label="轮播资讯名" prop="infoName">
        <el-input v-model.trim="informationForm.infoName" placeholder="请输入"></el-input>
      </el-form-item>
      <el-form-item label="上传图片" prop="picPath">
        ...
        >
          <img v-if="informationForm.picPath" :src="informationForm.picPath" class="avatar" />
          <i v-else class="el-icon-plus avatar-uploader-icon"></i>
          <div class="el-upload__tip" slot="tip">图片格式为654*318</div>
        </el-upload>
      </el-form-item>
      <el-form-item label="摘要" prop="infoAbbr">
        <el-input v-model.trim="informationForm.infoAbbr" placeholder="请输入"></el-input>
      </el-form-item>
      <el-form-item label="内容url" prop="infoUrl">
        <el-input v-model.trim="informationForm.infoUrl" placeholder="请输入"></el-input>
      </el-form-item>
      <el-form-item label="是否显示" prop="isShow">
        <el-select v-model="informationForm.isShow" clearable>
          <el-option
      ...
      <el-form-item>
        <el-button type="primary" @click="submitForm('informationForm')">提交</el-button>
...
<script>
import https from "http/informationManageApi";
...
export default {
  ...
  activated() {
    ...
    if (this.$route.query.informationForm && this.$route.query.type == "edit") {
      this.informationForm = JSON.parse(this.$route.query.informationForm);
      this.informationForm.access_token = this.access_token;
  ...
  methods: {
    ...
    handleAvatarSuccess(res, file, fileList) {
      ...
      this.productLoading = false;
      this.informationForm.picPath = res.payload.url;
      this.$message(`上传成功`);
    },
    submitForm(form) {
      let { informationForm } = this;
      let that = this;
      this.$refs[form].validate(valid => {
        if (valid) {
          if (that.pageType == "edit") {
            https.editInfomation(informationForm).then(data => {
              if (data.data.code == 0) {
                console.log(data);
                that.$router.back(-1);
                that.$message(`编辑成功`);
            ...
            https.addInformation(informationForm).then(data => {
              if (data.data.code == 0) {
                console.log(data);
                that.$router.back(-1);
                that.$message(`新增成功`);
