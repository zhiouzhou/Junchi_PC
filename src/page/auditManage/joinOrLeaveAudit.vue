  ...
  <div>
    ...
    <el-table :data="leaveList" border style="width: 100%;margin-top:14px;">
      ...
      <el-table-column label="审核类型">
        <template slot-scope="scope">
          <span>{{scope.row.authType===2?'技术人员加入门店申请':scope.row.authType===3?'技术人员离开门店申请':scope.row.authType===6?'门店申请开除技术员':''}}</span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    gotoAudit(row) {
      ...
      this.$router.push({
        name: `leaveInfo`,
        query: { authContent, applyId, pageType: "joinOrleave" }
      });
    },
    indexMethods(index) {
